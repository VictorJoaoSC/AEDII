...
class QuestaoRankingShellsort{
    ...
    public static int shellsortSort(){
        int h = 1;
    ...
    public static int insertionSort(int c , int h){
        ...
        for(int i = h + c; i < n; i+=h){
            ...
            while( (j>=0) && (lista[j].getEmpates() > temp.getEmpates() ) ){
                comparacoes++;
                lista[j+h] = lista[j];
                j-=h;
            }            
    ...
    public static void shellsortSortP(int c, int h){
    ...
    private static void criarTxt(long time, int comparacoes){
        String arq_name = "581203_shellsort.txt";
