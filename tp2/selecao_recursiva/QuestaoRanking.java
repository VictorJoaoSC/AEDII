...
class QuestaoRanking{
    ...
    private static long selectionSort(){
        ...
        selectionSort(0, lista);
        return System.currentTimeMillis() - inicio;
    }

    private static void selectionSort( int i, Time[] lista){
        
        int fim =  ranking.getQtdElementosLista();
        ...
        if(i!=fim-1){
            ...
            menor = selectionSort(i,i+1, menor, lista);

            swap(menor,i,lista);

            selectionSort(i+1,lista);
        }
        else
            comparacoes++;
    }

    private static int selectionSort(int i,int j,int menor, Time[] lista){

        int fim =  ranking.getQtdElementosLista();
        
        if(j < fim){
            comparacoes++;
             if(lista[menor].getPontos() > lista[j].getPontos()){
                 comparacoes++;
                 menor = j;
             }
            else{
                ...
                if(lista[menor].getPontos() == lista[j].getPontos()){
                    comparacoes++;
                    if(lista[menor].getPais().compareToIgnoreCase(lista[j].getPais()) > 0 ){
                        comparacoes++;
                        menor = j;
                    }else
                        comparacoes++;
           ...
           menor = selectionSort(i,j+1,menor,lista);
        }
        else
            comparacoes++;
    ...
    //ALGORTIMO DE COMPARAÇÃO DE STRINGs PARA ORDENAÇÃO ALFABETICA 
    ...
    private static void criarTxt(long tempo){
        ...
        try{
            arq = new FileWriter( new File("581203_selecaoRecursiva.txt"));
            arq.write(str);
            arq.close();
        }
        catch(Exception e){
