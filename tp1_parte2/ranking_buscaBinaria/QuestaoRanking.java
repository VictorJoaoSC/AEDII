...
class QuestaoRanking{
    ...
    //METODO RESPONSAVEL PELA LEITURA DO ARQUIVO DE ENTRA TXT
     private static void lerCopas() {
        ...
        while(pais.equals("FIM") == false){
            ...
            MyIO.println(pais+" -- "+(buscaBinaria(pais)?"SIM":"NÃO"));
    ...
    //METODO QUE RECEBE UM STRING QUE SÉRA A CHAVE DE BUSCA PARA ENCONTRAR OBJETOS NA LISTA
    //QUE POSSUI A MESMA CHAVE
    private static boolean buscaBinaria(String pais){
        ...
        while(e<=d && !presente){
            ...
            if(lista[m].getPais().equals(pais)){
                comparacoes++;
                presente = true;
            }
                ...
                comparacoes++;
                if(lista[m].getPais().compareToIgnoreCase(pais) < 0 ){
                    comparacoes++;
                    e = m+1;
                }
                else{
    ...
    private static void criarTxt(long time){
        String arq_name = "581203_binaria.txt";
