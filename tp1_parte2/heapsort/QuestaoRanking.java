...
class QuestaoRanking{
    ...
    private static void constroiHeap(int m, Time[] lista){
        ...
        for( k = 1; k < m-1; k++){
            ...
            while(f > 1 && lista[f/2].getDerrotas() < lista[f].getDerrotas()){
                swap(f/2,f,lista);
    ...
    private static void peneira(int m,Time[] lista ){
        ...
        while(f< m-1){
            if(f < m && lista[f].getDerrotas() < lista[f+1].getDerrotas())
                f++;
            if(temp.getDerrotas() >= lista[f].getDerrotas())
                break;
            lista[p] = lista[f];
    ...
    private static int getMaiorFilho(int i, int tam,Time[] lista){
        int filho;

        if(2*i == tam || lista[2*1].getDerrotas() > lista[2*i+1].getDerrotas() )
            filho = 2*i;
        else
            filho = 2*i+1;
    ...
    //ORDENAÇÃO
    private static void criarTxt(long time, int comparacoes){
